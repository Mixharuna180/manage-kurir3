// server-legacy.js - Server Express yang kompatibel dengan Node.js versi lama
...
app.get('/api/health', function(req, res) {
  res.json({
    ...
    message: 'Server berjalan dengan baik (Legacy Version)'
  });
});
  ...
  pool.query('SELECT NOW()', function(err, result) {
    if (err) {
      ...
      res.json({
        ...
        server_version: 'server-legacy.js'
      });
    }
  });
});
...
function ensureIndexHtml() {
  ...
  if (!fs.existsSync(indexPath)) {
    ...
    var html = '<!DOCTYPE html>\n' +
      ...
      '      <p>This is a legacy compatible server version.</p>\n' +
      '    </div>\n' +
      '    <h3>API Endpoints:</h3>\n' +
      '    <ul>\n' +
...
var server = app.listen(port, '0.0.0.0', function() {
  console.log('----------------------------------------');
  console.log('LogiTech Legacy Server v1.0');
  console.log('----------------------------------------');
