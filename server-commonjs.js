// server-commonjs.js - Server Express yang kompatibel dengan Node.js versi lama (CommonJS)
  ...
  pool.query('SELECT NOW()', function(err, result) {
    if (err) {
      ...
      res.json({
        ...
        server_version: 'server-commonjs.js'
      });
    }
  });
});
...
function ensureIndexHtml() {
  ...
  if (!fs.existsSync(indexPath)) {
    ...
    var html = '<!DOCTYPE html>\n' +
      ...
      '      <p>This is a CommonJS compatible server version.</p>\n' +
      '    </div>\n' +
      '    <h3>API Endpoints:</h3>\n' +
      '    <ul>\n' +
...
var server = app.listen(port, '0.0.0.0', function() {
  console.log('----------------------------------------');
  console.log('LogiTech CommonJS Server v1.0');
  console.log('----------------------------------------');
